...
// 异步相关
constexpr size_t STACK_BUFFER_SIZE = 1024; // 必须小于等于CURRENT_BUFFER_SIZE 这个大小也就是一条日志的最大长度(包括自动补充的时间等字符)
constexpr size_t CURRENT_BUFFER_SIZE = 2000; // 当前缓冲区固定大小 每次写入文件的字节数不会超出这个大小 要求小于等于MAX_LOGFILE_SIZE
...
constexpr const char *DEFAULT_TIME_ZONE = "Asia/Shanghai"; // 默认的日志时区
